    ...
    <div className="w-full px-4 space-y-4 flex flex-col gpu-layer pb-40 min-h-full">
      ...
      <section className="space-y-4">
        <RecordList 
          ...
          copyFooter={settings.copyFooter}
        />
      </section>
...
const App: React.FC = () => {
  ...
  const [hasAuthenticatedForHistoryDelete, setHasAuthenticatedForHistoryDelete] = useState(false);
  const [deletedActionItem, setDeletedActionItem] = useState<any>(null);
  
  const [passcodeContext, setPasscodeContext] = useState<{ 
    action: 'clear' | 'deleteTab' | 'addRecord' | 'editRecord' | 'deleteRecord' | 'adjustQty' | 'toggleRestriction' | 'openUsers' | 'push' | 'pull' | 'setupPersonalCloud' | 'deleteCycle' | 'deleteInvestor' | 'deleteSignature' | 'deleteHistory' | 'deleteActionHistory'; 
    targetTab?: string;
    data?: any;
  ...
  const [initialRecentActions, setInitialRecentActions] = useState<any[]>([]);
  ...
  const [startupQueue, setStartupQueue] = useState<{id: string, title: string, details: React.ReactNode, type?: string, tabName?: string, meta?: any}[]>([]);
  const [historyScrubQueue, setHistoryScrubQueue] = useState<string[]>([]);
  const [currentStartupNotification, setCurrentStartupNotification] = useState<{id: string, title: string, details: React.ReactNode, type?: string, tabName?: string, meta?: any} | null>(null);
  const hasCheckedStartupRef = useRef(false);

  useEffect(() => {
    if (!isInitialLoad && session && !hasCheckedStartupRef.current && !toast.visible && !isRefreshing && historyScrubQueue.length === 0) {
      hasCheckedStartupRef.current = true;
      
      const queue: {id: string, title: string, details: React.ReactNode, type?: 'contract' | 'rent' | 'investor' | 'inventory', tabName?: string, meta?: any}[] = [];
      ...
      if (session.role === 'master') {
        ...
        Object.entries(allRecords).forEach(([tabName, records]) => {
          ...
          const newContractsDetails: {name: string, date: string}[] = [];
          
          records.forEach(r => {
            if (r.signature && r.signatureDate) {
              if (isToday(new Date(r.signatureDate))) {
                newContractsCount++;
                newContractsDetails.push({ name: r.name, date: r.signatureDate });
              }
            }
          });
          
          if (newContractsCount > 0) {
            newContractsDetails.sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
            const displayDetails = newContractsDetails.slice(0, 3).map(d => {
              const dObj = new Date(d.date);
              const timeStr = dObj.toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'});
              return `${d.name} (${timeStr})`;
            });
            
            if (newContractsCount > 3) {
              displayDetails.push(`...and ${newContractsCount - 3} more`);
            }

            const tabType: any = settings.tabTypes[tabName];
            let icon = '📝';
            if (tabType === 'rent') icon = '🚗';
            else if (tabType === 'debt') icon = '💵';
            else if (tabType === 'investor') icon = '💼';

            const id = `contract-${tabName}`;
            const latestSigDate = newContractsDetails.reduce((max, curr) => curr.date > max ? curr.date : max, "");
            const lastAckSig = localStorage.getItem(`ack-sig-${id}`);
            
            if (lastAckSig !== latestSigDate) {
              queue.push({
                id,
                title: `${icon} ${newContractsCount} new contract${newContractsCount > 1 ? 's' : ''} signed today!`,
                details: displayDetails.join('\n'),
                type: 'contract',
                tabName: `Tab: ${tabName}`,
                meta: latestSigDate // Store the timestamp to acknowledge later
              });
            }
          }
        });
        
        let investorContractsCount = 0;
        const investorContractsDetails: {name: string, date: string}[] = [];
        investors.forEach(inv => {
          if (inv.signature && inv.signatureDate) {
            if (isToday(new Date(inv.signatureDate))) {
              investorContractsCount++;
              investorContractsDetails.push({ name: inv.name, date: inv.signatureDate });
            }
          }
        });
        
        if (investorContractsCount > 0) {
          investorContractsDetails.sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
          const displayDetails = investorContractsDetails.slice(0, 3).map(d => {
            const dObj = new Date(d.date);
            const timeStr = dObj.toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'});
            return `${d.name} (${timeStr})`;
          });
          
          if (investorContractsCount > 3) {
            displayDetails.push(`...and ${investorContractsCount - 3} more`);
          }

          const id = 'contract-investors';
          const latestSigDate = investorContractsDetails.reduce((max, curr) => curr.date > max ? curr.date : max, "");
          const lastAckSig = localStorage.getItem(`ack-sig-${id}`);

          if (lastAckSig !== latestSigDate) {
            queue.push({
              id,
              title: `💼 ${investorContractsCount} new contract${investorContractsCount > 1 ? 's' : ''} signed today!`,
              details: displayDetails.join('\n'),
              type: 'contract',
              tabName: 'Tab: Investors',
              meta: latestSigDate
            });
          }
        }
      ...
      Object.entries(allRecords).forEach(([tabName, records]) => {
        if (session.role !== 'master' && !visibleTabs.includes(tabName)) return;
        if (settings.tabTypes[tabName] === 'rent') {
          let rentalsToday = 0;
          const rentalsTodayDetails: string[] = [];
          let rentalsTomorrow = 0;
          const rentalsTomorrowDetails: string[] = [];

          records.forEach(r => {
            if (r.status !== 'finished' && r.status !== 'cancelled' && r.status !== 'deleted') {
              if (r.date === todayStr) {
                rentalsToday++;
                rentalsTodayDetails.push(`${r.name} (${r.date})`);
              } else if (r.date === tomorrowStr) {
                rentalsTomorrow++;
                rentalsTomorrowDetails.push(`${r.name} (${r.date})`);
              }
            }
          });

          if (rentalsToday > 0) {
            const id = `rent-today-${tabName}`;
            if (localStorage.getItem(`ack-notif-${id}`) !== todayStr) {
              queue.push({
                id,
                title: `🚗 ${rentalsToday} rental${rentalsToday > 1 ? 's' : ''} scheduled to start today.`,
                details: rentalsTodayDetails.join('\n'),
                type: 'rent',
                tabName: `Tab: ${tabName}`
              });
            }
          }
          if (rentalsTomorrow > 0) {
            const id = `rent-tomorrow-${tabName}`;
            if (localStorage.getItem(`ack-notif-${id}`) !== todayStr) {
              queue.push({
                id,
                title: `🚗 ${rentalsTomorrow} rental${rentalsTomorrow > 1 ? 's' : ''} scheduled to start tomorrow.`,
                details: rentalsTomorrowDetails.join('\n'),
                type: 'rent',
                tabName: `Tab: ${tabName}`
              });
            }
      ...
      // 3. Investor payouts
      if (session.role === 'master') {
        let investorPayoutsToday = 0;
        const investorPayoutsTodayDetails: React.ReactNode[] = [];
        let investorPayoutsTomorrow = 0;
        const investorPayoutsTomorrowDetails: React.ReactNode[] = [];
        investors.forEach(inv => {
          ...
          while (monthsToAdd < 120) {
            const tempDate = new Date(investDate);
            tempDate.setHours(0, 0, 0, 0);
            tempDate.setMonth(tempDate.getMonth() + monthsToAdd);
            
            if (tempDate.getTime() >= now.getTime()) {
              const payoutStr = `${tempDate.getFullYear()}-${String(tempDate.getMonth() + 1).padStart(2, '0')}-${String(tempDate.getDate()).padStart(2, '0')}`;
              if (payoutStr === todayStr) {
                investorPayoutsToday++;
                investorPayoutsTodayDetails.push(
                  <div key={inv.id} className="mb-2 last:mb-0">
                    <div>{inv.name} ({inv.dateInvested})</div>
                    {inv.notes && (
                      <div className="mt-1 p-2 rounded-lg bg-gradient-to-r from-yellow-500/20 to-amber-500/10 border border-yellow-500/20 text-yellow-200 text-xs leading-relaxed whitespace-pre-wrap">
                        📝 {inv.notes}
                      </div>
                    )}
                  </div>
                );
              } else if (payoutStr === tomorrowStr) {
                investorPayoutsTomorrow++;
                investorPayoutsTomorrowDetails.push(
                  <div key={inv.id} className="mb-2 last:mb-0">
                    <div>{inv.name} ({inv.dateInvested})</div>
                    {inv.notes && (
                      <div className="mt-1 p-2 rounded-lg bg-gradient-to-r from-yellow-500/20 to-amber-500/10 border border-yellow-500/20 text-yellow-200 text-xs leading-relaxed whitespace-pre-wrap">
        ...
        if (investorPayoutsToday > 0) {
          const id = 'investor-today';
          if (localStorage.getItem(`ack-notif-${id}`) !== todayStr) {
            queue.push({
              id,
              title: `💰 ${investorPayoutsToday} investor payout${investorPayoutsToday > 1 ? 's' : ''} due today.`,
              details: <div>{investorPayoutsTodayDetails}</div>,
              type: 'investor'
            });
          }
        }
        if (investorPayoutsTomorrow > 0) {
          const id = 'investor-tomorrow';
          if (localStorage.getItem(`ack-notif-${id}`) !== todayStr) {
            queue.push({
              id,
              title: `💰 ${investorPayoutsTomorrow} investor payout${investorPayoutsTomorrow > 1 ? 's' : ''} due tomorrow.`,
              details: <div>{investorPayoutsTomorrowDetails}</div>,
              type: 'investor'
            });
          }
        }
      ...
      // 4. Supply and Product Understock/Overstock
      Object.entries(allRecords).forEach(([tabName, records]) => {
        if (session.role !== 'master' && !visibleTabs.includes(tabName)) return;
        const type = settings.tabTypes[tabName];
        if (type === 'supply' || type === 'product') {
          let understockCount = 0;
          const understockDetails: string[] = [];
          let overstockCount = 0;
          const overstockDetails: string[] = [];

          records.forEach(r => {
            if (r.status !== 'finished' && r.status !== 'cancelled' && r.status !== 'deleted') {
              const qty = r.amount || 0;
              if (r.minAmount !== undefined && qty < r.minAmount) {
                understockCount++;
                understockDetails.push(`${r.name} (${qty} < ${r.minAmount})`);
              }
              if (r.maxAmount !== undefined && qty > r.maxAmount) {
                overstockCount++;
                overstockDetails.push(`${r.name} (${qty} > ${r.maxAmount})`);
              }
            }
          });

          if (understockCount > 0) {
            const id = `inventory-understock-${tabName}`;
            if (localStorage.getItem(`ack-notif-${id}`) !== todayStr) {
              queue.push({
                id,
                title: `💼 ${understockCount} item${understockCount > 1 ? 's' : ''} understocked!`,
                details: understockDetails.join('\n'),
                type: 'inventory',
                tabName: `Tab: ${tabName}`
              });
            }
          ...
          if (overstockCount > 0) {
            const id = `inventory-overstock-${tabName}`;
            if (localStorage.getItem(`ack-notif-${id}`) !== todayStr) {
              queue.push({
                id,
                title: `💼 ${overstockCount} item${overstockCount > 1 ? 's' : ''} overstocked!`,
                details: overstockDetails.join('\n'),
                type: 'inventory',
                tabName: `Tab: ${tabName}`
              });
            }
      ...
      if (queue.length > 0) {
        setStartupQueue(queue);
      }
    }
  }, [isInitialLoad, session, allRecords, investors, settings.tabTypes, toast.visible, isRefreshing, historyScrubQueue.length, visibleTabs]);

  useEffect(() => {
    if (startupQueue.length > 0 && !currentStartupNotification) {
      const nextMsg = startupQueue[0];
      setCurrentStartupNotification(nextMsg);
      setStartupQueue(prev => prev.slice(1));
    }
  }, [startupQueue, currentStartupNotification]);

  useEffect(() => {
    if (currentStartupNotification) {
      // Acknowledge the notification
      if (currentStartupNotification.type === 'contract' && currentStartupNotification.meta) {
        localStorage.setItem(`ack-sig-${currentStartupNotification.id}`, String(currentStartupNotification.meta));
      } else {
        localStorage.setItem(`ack-notif-${currentStartupNotification.id}`, getTodayStr());
      }

      const timer = setTimeout(() => {
        setCurrentStartupNotification(null);
      }, 8000);
      return () => clearTimeout(timer);
    }
  }, [currentStartupNotification]);
  ...
  const executeDeleteInvestor = async (id: string) => {
    ...
    showToast("Deleting investor entry...");
  ...
  const handleDeleteInvestor = async (id: string) => {
    setPasscodeContext({ action: 'deleteInvestor', data: { id } });
  ...
  const executeManualHistoryDelete = useCallback(async (record: DebtRecord, scrubPerson: boolean) => {
    ...
    if (scrubPerson) {
        setIsRefreshing(true);
        setRefreshIsBlocking(true);
        showToast("Deleting Deleted Person...");
  ...
  const handleManualHistoryDelete = useCallback(async (record: DebtRecord, scrubPerson: boolean) => {
    if (!hasAuthenticatedForHistoryDelete) {
      setPasscodeContext({ action: 'deleteHistory', data: { record, scrubPerson } });
      setIsPasscodeModalOpen(true);
      return;
    }
    await executeManualHistoryDelete(record, scrubPerson);
  }, [hasAuthenticatedForHistoryDelete, executeManualHistoryDelete]);

  const executeDeleteActionHistory = useCallback(async (actionItem: any) => {
    const activeUrl = isOfflineMode ? '' : settings.scriptUrl;
    
    // Optimistically remove from local initialRecentActions
    setInitialRecentActions(prev => prev.filter(a => a.timestamp !== actionItem.timestamp || a.details !== actionItem.details));
    setDeletedActionItem(actionItem);
    
    if (!activeUrl) return;

    setIsRefreshing(true);
    setRefreshIsBlocking(true);
    showToast("Deleting Recent Action...");

    try {
      const res = await fetch(activeUrl, {
        method: 'POST',
        body: JSON.stringify({ action: 'deleteActionHistory', timestamp: actionItem.timestamp, details: actionItem.details })
      });
      const data = await res.json();
      if (data.status === 'success' || (data.message && data.message.toLowerCase().includes('not found'))) {
        showToast("Action history deleted");
      } else {
        showToast(data.message || "Failed to delete action history", "error");
      }
    } catch (e) {
      showToast("Failed to delete action history", "error");
    } finally {
      setIsRefreshing(false);
    }
  }, [isOfflineMode, settings.scriptUrl, showToast]);

  const handleDeleteActionHistory = useCallback(async (actionItem: any) => {
    if (!hasAuthenticatedForHistoryDelete) {
      setPasscodeContext({ action: 'deleteActionHistory', data: actionItem });
      setIsPasscodeModalOpen(true);
      return;
    }
    await executeDeleteActionHistory(actionItem);
  }, [hasAuthenticatedForHistoryDelete, executeDeleteActionHistory]);

  const generateActionDetail = useCallback((oldRecord: DebtRecord | undefined, newRecord: DebtRecord, tabName?: string): string => {
    const tabType = tabName ? settings.tabTypes[tabName] : (newRecord.tab ? settings.tabTypes[newRecord.tab] : undefined);

    if (!oldRecord) {
      if (tabType === 'debt') return `added new debt entry ${newRecord.name}`;
      if (tabType === 'rent') return `added new rent entry ${newRecord.name}`;
      if (tabType === 'supply' || tabType === 'product') {
        const typeLabel = newRecord.transactionType === 'income' ? 'incoming' : 'outgoing';
        return `added new ${typeLabel} entry ${newRecord.name}`;
      }
      if (tabType === 'cashflow') {
        const typeLabel = newRecord.transactionType === 'income' ? 'incoming' : 'outgoing';
        return `added new ${typeLabel} entry ${newRecord.name}`;
      }
      if (tabType === 'salary') return `added new salary entry ${newRecord.name}`;
      if (tabType === 'business') {
        if (newRecord.businessEntryType === 'capital') return `Started cycle ${newRecord.name} initial capital - ${newRecord.amount}`;
        if (newRecord.businessEntryType === 'expense') return `added new expenses entry for ${newRecord.name} amount ${newRecord.amount}`;
        if (newRecord.businessEntryType === 'earning') return `Ended cycle for ${newRecord.name} cash on hand amount ${newRecord.cashOnHand || newRecord.amount}`;
        return `added new business entry ${newRecord.name}`;
      }
      if (tabType === 'savings') return `added new savings entry ${newRecord.name}`;
      if (tabType === 'sales') {
        if (newRecord.salesEntryType === 'capital') return `added new capital entry ${newRecord.name} - ${newRecord.amount}`;
        if (newRecord.salesEntryType === 'expense') return `added new expenses entry ${newRecord.name} - ${newRecord.amount}`;
        if (newRecord.salesEntryType === 'cycle_end') return `Ended cycle for ${newRecord.name}`;
        return `added new sales entry ${newRecord.name}`;
      }
      
      return `added new entry ${newRecord.name}`;
    }
    
    if (oldRecord.name !== newRecord.name) {
      return `Renamed ${oldRecord.name} entry to ${newRecord.name}`;
    }
    
    if (oldRecord.status !== newRecord.status) {
      if (newRecord.status === 'cancelled') return `cancelled booking for ${newRecord.name}`;
      if (newRecord.status === 'finished') return `finished booking for ${newRecord.name}`;
      if (newRecord.status === 'active') return `reactivated booking for ${newRecord.name}`;
      return `changed status for ${newRecord.name} to ${newRecord.status}`;
    }
    
    if (oldRecord.date !== newRecord.date) {
      return `change the date for ${newRecord.name} from ${oldRecord.date} to ${newRecord.date}`;
    }
    
    if (oldRecord.remarks !== newRecord.remarks) {
      return `change the remark for ${newRecord.name} from ${oldRecord.remarks || 'empty'} to ${newRecord.remarks || 'empty'}`;
    }
    
    if (oldRecord.amount !== newRecord.amount) {
      const isInventory = tabType === 'supply' || tabType === 'product';
      const label = isInventory ? 'quantity' : 'amount';
      return `change ${label} for ${newRecord.name} from ${oldRecord.amount} to ${newRecord.amount}`;
    }

    return `edited entry for ${newRecord.name}`;
  }, [settings.tabTypes]);

  const logRecentActionFrontend = useCallback(async (details: string, tabName: string) => {
    if (tabName === 'Investors' || tabName === 'Tab: Investors') return;
    const activeUrl = isOfflineMode ? '' : settings.scriptUrl;
    
    // Optimistically update the local state so it appears immediately in HistoryModal
    setInitialRecentActions(prev => {
      const newAction = { timestamp: new Date().toISOString(), details: details, tabName: tabName };
    ...
    try {
      await fetch(activeUrl, {
        method: 'POST',
        body: JSON.stringify({ action: 'logRecentAction', details: details, tabName: tabName })
      });
    } catch (e) {
      console.error("Failed to log action", e);
    }
  }, [isOfflineMode, settings.scriptUrl]);
        ...
        if (data.status === 'success') {
            ...
            if (data.investors) {
                setInvestors(prev => {
                    ...
                    prev.forEach(inv => { if (inv.signature) sigMap.set(inv.id, inv); });
                    return data.investors.map((inv: Investor) => {
                        const s = sigMap.get(inv.id);
                        return s ? { 
                            ...inv, 
  ...
  const executeDeleteSignature = useCallback(async (id: string, type: 'record' | 'investor', tabName?: string) => {
    setIsRefreshing(true);
    setRefreshIsBlocking(true);
    showToast("Deleting signature...");
    ...
    try {
      ...
      if (type === 'record' && tabName) {
        ...
        setInvestors(prev => prev.map(inv => inv.id === id ? { 
            ...inv, 
            signature: undefined, 
            signatureDate: undefined,
            signerName: undefined,
        ...
        setInvestorContractToView(null);
      }
  ...
  const handleDeleteSignature = useCallback(async (id: string, type: 'record' | 'investor', tabName?: string) => {
    setPasscodeContext({ action: 'deleteSignature', data: { id, type, tabName } });
    setIsPasscodeModalOpen(true);
  }, []);

  const fetchAllData = useCallback(async (silent = false, fullSync = false, targetTabOverride?: string, customScriptUrl?: string, limit?: number, offset?: number) => {
    ...
    try {
      ...
      if (offset !== undefined) urlParams.append('offset', offset.toString());
      ...
      if (data.pendingDraftIds) setPendingDraftIds(data.pendingDraftIds);
      if (data.initialRecentActions) setInitialRecentActions(data.initialRecentActions);
      ...
      if (data.signatures) {
        ...
        localStorage.setItem('known_signatures', JSON.stringify(currentSigIds));
         ...
         // Merge local signatures into incomingInvestors
         setInvestors(prevInvestors => {
             const invSigMap = new Map();
             prevInvestors.forEach(inv => { if (inv.signature) invSigMap.set(inv.id, inv); });
             return incomingInvestors.map((inv: Investor) => {
                 const s = invSigMap.get(inv.id);
                 if (s) {
                     return { 
                         ...inv, 
                         signature: s.signature, 
  ...
  const handleUpdateRecordInline = async (record: DebtRecord) => {
    ...
    if (activeUrl) {
      ...
      const oldRecord = previousRecords.find(r => r.id === record.id);
      setAllRecords(prev => ({ ...prev, [tabName]: (prev[tabName] || []).map(r => r.id === record.id ? record : r) }));

      const performInlineUpdate = async () => {
          setIsRefreshing(true); 
          setRefreshIsBlocking(false);
          logRecentActionFrontend(generateActionDetail(oldRecord, record, tabName), tabName);
          try {
  ...
  const handleRecordSubmit = async (recordData: DebtRecord | DebtRecord[], isEdit: boolean, sideEffectRecord?: any) => {
    ...
    const previousRecords = allRecords[tabName] || [];
    const oldRecord = isEdit ? previousRecords.find(r => r.id === enrichedItems[0].id) : undefined;
    setAllRecords(prev => {
      if (isEdit) {
        const editIds = new Set(enrichedItems.map(r => r.id));
        return { 
    ...
    const performSubmit = async () => {
        setIsRefreshing(true); 
        logRecentActionFrontend(generateActionDetail(oldRecord, enrichedItems[0], tabName), tabName);
  ...
  const handleLocalTabCopy = useCallback((filter: string, tabName: string) => {
    ...
    const bullet = settings.copyBullet || '🌸';
    const dateBullet = settings.copyDateBullet || '📌';
    const footer = settings.copyFooter || 'Thank you - Lmk';
      ...
      if (filter === 'overdue') {
        ...
        if (overdueItems.length > 0) {
          ...
          sortedDates.forEach(date => { 
            ...
            text += `${dateBullet}'${formatDateMD(date)} Over due ${diffDays}d\n`; 
        ...
        if (todayItems.length > 0) { 
          ...
          text += `${dateBullet}'${formatDateMD(today).toUpperCase()} Due Today\n`; 
        ...
        text = `(Due for tommorow)\nSummary for\n\n✨${tabName.toUpperCase()}✨ \n\nAs of: ${asOf}\n\n${dateBullet}'DUE TOMMORROW: ${formatDateMD(tomorrowStr).toUpperCase()}\n\n`;
        ...
        const sortedDates = Object.keys(groupedByDate).sort();
        sortedDates.forEach(date => { 
            let dateHeader = `${dateBullet}'${formatDateMD(date)}`; 
            if (date < today) { 
  ...
  const handleCopyAlerts = useCallback(() => {
    ...
    debtTabs.forEach(tabName => {
      ...
      if (overdue.length > 0 || tRecs.length > 0 || tmRecs.length > 0) {
        ...
        if (overdue.length > 0) {
          alertText += `${dateBullet}'OVER DUE SECTION:\n\n`;
        ...
        if (tRecs.length > 0) {
          alertText += `${dateBullet}'DUE TODAY SECTION: \n     ${formatDateMD(todayStr).toUpperCase()}\n\n`;
        ...
        if (tmRecs.length > 0) {
          alertText += `${dateBullet}'DUE TOMORROW SECTION:\n      ${formatDateMD(tomorrowStr).toUpperCase()}\n\n`;
  ...
  const handleAuthSuccess = async (code: string) => {
    ...
    if (code === "BIOMETRIC_PASS" || code === validPin || passcodeContext?.action === 'setupPersonalCloud') {
      const currentContext = passcodeContext;
      setIsPasscodeModalOpen(false); 
      setPasscodeContext(null);

      if (currentContext) {
        const { action, targetTab, data, nextAction } = currentContext as any;
        switch (action) {
          ...
          case 'deleteInvestor': {
            await executeDeleteInvestor(data.id);
            break;
          }
          case 'deleteSignature': {
            await executeDeleteSignature(data.id, data.type, data.tabName);
            break;
          }
          case 'deleteHistory': {
            setHasAuthenticatedForHistoryDelete(true);
            await executeManualHistoryDelete(data.record, data.scrubPerson);
            break;
          }
          case 'deleteActionHistory': {
            setHasAuthenticatedForHistoryDelete(true);
            await executeDeleteActionHistory(data);
            break;
          }
  ...
  const handleDeleteRecord = async () => {
    ...
    const shouldSaveToHistory = (tabType === 'debt' || tabType === 'rent');
    if (shouldSaveToHistory) {
      setSettings(prev => ({ ...prev, deletedHistory: [...prev.deletedHistory, recordToHistory] })); 
    } 
    logRecentActionFrontend(`deleted entry for ${targetRecord.name}`, currentTab);
  ...
  const handleClearTab = async (name: string) => {
    const activeUrl = isOfflineMode ? settings.personalScriptUrl : settings.scriptUrl; setIsRefreshing(true); setRefreshIsBlocking(true);
  ...
  const handlePushToServer = async () => { 
    ...
    try { 
      ...
      await fetch(activeUrl, { 
        method: 'POST', 
        headers: { 'Content-Type': 'text/plain;charset=utf-8' },
        body: JSON.stringify({ action: 'reorderTabs', tabs: tabs, pin: pin }) 
      }).catch(err => console.warn("Failed to sync tab order", err));
  ...
  return (
    <div className="h-full max-w-lg mx-auto bg-slate-50 relative flex flex-col overflow-hidden" onTouchStart={handleTouchStart} onTouchMove={handleTouchMove} onTouchEnd={handleTouchEnd}>
      ...
      <div className="fixed top-8 left-0 right-0 z-[140000] flex justify-center pointer-events-none">
        <AnimatePresence mode="wait">
          {currentStartupNotification && (
            <motion.div
              key={currentStartupNotification.title}
            ...
            >
              <div className={`px-5 py-3 rounded-2xl shadow-[0_20px_50px_rgba(0,0,0,0.5)] flex flex-col gap-1 border backdrop-blur-2xl text-white relative pr-10 ${
                currentStartupNotification.type === 'contract' ? 'bg-emerald-900/95 border-emerald-500/50' :
                currentStartupNotification.type === 'investor' ? 'bg-black/95 border-gray-700/50' :
                currentStartupNotification.type === 'inventory' ? 'bg-slate-800/95 border-slate-600/50' :
                'bg-indigo-900/95 border-indigo-500/50'
              }`}>
                <button 
                  onClick={() => setCurrentStartupNotification(null)}
                  className={`absolute top-3 right-3 transition-colors ${
                    currentStartupNotification.type === 'contract' ? 'text-emerald-300 hover:text-white' :
                    currentStartupNotification.type === 'investor' ? 'text-gray-400 hover:text-white' :
                    currentStartupNotification.type === 'inventory' ? 'text-slate-400 hover:text-white' :
                ...
                <span className="text-sm font-black tracking-wide whitespace-nowrap">{currentStartupNotification.title}</span>
                {currentStartupNotification.tabName && (
                  <span className={`text-base font-bold tracking-wide whitespace-nowrap block text-center ${
                    currentStartupNotification.type === 'contract' ? 'text-emerald-100' :
                    currentStartupNotification.type === 'investor' ? 'text-gray-100' :
                    currentStartupNotification.type === 'inventory' ? 'text-slate-100' :
                    'text-indigo-100'
                  }`}>{currentStartupNotification.tabName}</span>
                )}
                {currentStartupNotification.details && (
                  <div className={`text-xs font-medium whitespace-pre-wrap max-h-32 overflow-y-auto no-scrollbar ${
                    currentStartupNotification.type === 'contract' ? 'text-emerald-200' :
                    currentStartupNotification.type === 'investor' ? 'text-gray-300' :
                    currentStartupNotification.type === 'inventory' ? 'text-slate-300' :
                    'text-indigo-200'
                  }`}>{currentStartupNotification.details}</div>
                )}
                {['rent', 'investor', 'inventory'].includes(currentStartupNotification.type || '') && (
                  <button
                    onClick={(e) => {
                      e.stopPropagation();
                      localStorage.setItem(`ack-notif-${currentStartupNotification.id}`, getTodayStr());
                    ...
                    className={`mt-2 py-1.5 px-3 rounded-lg text-xs font-bold transition-colors w-full ${
                      currentStartupNotification.type === 'investor' ? 'bg-gray-800 hover:bg-gray-700 text-gray-200' :
                      currentStartupNotification.type === 'inventory' ? 'bg-slate-700 hover:bg-slate-600 text-slate-200' :
      ...
      {isHistoryOpen && <HistoryModal onClose={() => setIsHistoryOpen(false)} history={settings.deletedHistory} tabType={settings.tabTypes[activeTab] || 'debt'} tabName={activeTab} activeUrl={(isOfflineMode ? settings.personalScriptUrl : settings.scriptUrl) || ''} showToast={showToast} initialRecentActions={initialRecentActions} isOfflineMode={isOfflineMode} deletedActionItem={deletedActionItem} onReuse={(r) => { setEditingRecord({ ...r, id: '', status: 'active', date: getTodayStr() }); setIsFormOpen(true); setIsHistoryOpen(false); }} onEdit={(r) => { setEditingRecord(r); setIsFormOpen(true); setIsHistoryOpen(false); }} onDelete={(r) => { setDeletingRecordData({ id: r.id }); setIsHistoryOpen(false); }} onDeleteFromHistory={handleManualHistoryDelete} onDeleteActionHistory={handleDeleteActionHistory} onViewContract={(r) => setContractRecord(r)} isMaster={isMaster} />}
      <AnimatePresence>
      ...
      {rentalSummaryData && <RentalSummaryModal isOpen={true} onClose={() => setRentalSummaryData(null)} records={rentalSummaryData.records} tab={rentalSummaryData.tab} totalYearEarnings={rentalSummaryData.total} showToast={showToast} copyBullet={settings.copyBullet || '🌸'} copyFooter={settings.copyFooter || 'Thank you - Lmk'} />}
      ...
      {isInvestorModalOpen && <InvestorModal isOpen={true} onClose={() => setIsInvestorModalOpen(false)} investors={investors} pendingDraftIds={pendingDraftIds} onAddInvestor={handleAddInvestor} onUpdateInvestor={handleUpdateInvestor} onDeleteInvestor={handleDeleteInvestor} onDeleteSignature={(id, type) => handleDeleteSignature(id, type)} onGenerateLink={(id) => setPendingDraftIds(prev => prev.includes(id) ? prev : [...prev, id])} onOpenContract={(investor) => setInvestorContractToView(investor)} currencyConfig={settings.currencyConfigs?.[activeTab]} scriptUrl={isOfflineMode ? settings.personalScriptUrl : settings.scriptUrl} githubPagesUrl={settings.githubPagesUrl} showToast={showToast} authorizedSignature={settings.authorizedSignature} fundHolderName={settings.fundHolderName} />}
