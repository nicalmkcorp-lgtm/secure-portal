...
const ContractModal: React.FC<ContractModalProps> = ({ isOpen, onClose, record, tabType, scriptUrl, githubPagesUrl, currencySymbol, authorizedSignature, lenderName, operatorName: propOperatorName, onDeleteSignature, onSaveContractDetails, biometricEnabled, onGenerateLink }) => {
  ...
  const handlePrint = async () => {
    if (Capacitor.isNativePlatform()) {
      ...
      const title = isRent ? 'Rental Agreement' : 'Loan Acknowledgement Form';
      await generateContractPDF(title, termsText, record?.signature, displayName, record?.signatureDate ? formatDateMedium(record.signatureDate) : undefined, metaData, operatorName, authorizedSignature);
    }
  };
      ...
      {isRent ? (
        <div className="grid grid-cols-2 gap-3">
          <div className="space-y-1.5">
            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Renting Price Amount ({currencySymbol})</label>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="space-y-1.5">
            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Loan Amount ({currencySymbol})</label>
            <input type="number" value={amount} onChange={e => setAmount(e.target.value)} className="w-full p-4 bg-slate-50 border border-slate-200 rounded-2xl font-bold text-slate-800 text-sm outline-none focus:ring-2 focus:ring-blue-500" />
          </div>
          {term !== 'One payment' && (
            <div className="space-y-1.5">
            ...
            {record?.signature ? (
              <div className="space-y-6">
                <div className="bg-white border border-slate-200 p-8 shadow-sm overflow-hidden flex flex-col font-serif text-slate-800 min-h-[500px]">
                  ...
                  <div className="flex justify-between items-end gap-4 mt-auto pt-8 border-t border-slate-100">
                      ...
                      <div className="flex-1 flex flex-col items-center">
                         ...
                         {record.signatureDate && <span className="text-[6px] italic opacity-60 mt-0.5">Signed: {formatDateMedium(record.signatureDate)}</span>}
                      </div>
                  </div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  ...
                  <button onClick={() => { if (onDeleteSignature) onDeleteSignature(record?.id, 'record', record?.tab || tabType); }} className="py-3 bg-rose-50 border border-rose-100 text-rose-600 font-bold rounded-2xl active:scale-95 transition-all text-[10px] uppercase tracking-widest flex items-center justify-center gap-2 hover:bg-rose-100">
