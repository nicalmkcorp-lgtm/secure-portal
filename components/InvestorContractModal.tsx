...
1. CAPITAL & RETURN
The Investor agrees to invest the Total Investment as stated in this agreement. In return, the Fund Holder agrees to pay monthly returns (interest or dividends) as specified in the investment particulars.
...
This agreement is binding for the duration of the investment period. Any withdrawal of the Total Investment requested by the Investor requires a formal notice period of at least thirty (30) days prior to the intended withdrawal date.

4. RIGHT OF RETURN (FUND HOLDER)
The Fund Holder reserves the right, at its sole discretion, to return the full Total Investment to the Investor at any time. This provision may be exercised should underlying business circumstances, operational shifts, or unforeseen internal factors necessitate the termination of this investment arrangement.
...
const InvestorContractModal: React.FC<InvestorContractModalProps> = ({ isOpen, onClose, investor, scriptUrl, githubPagesUrl, currencyCode, authorizedSignature, fundHolderName: propFundHolderName, onDeleteSignature, onGenerateLink, biometricEnabled }) => {
  ...
  const [investorAddress, setInvestorAddress] = useState(() => investor?.signerAddress || '');
  ...
  const [dateInvested, setDateInvested] = useState(() => (investor?.dateInvested || new Date().toISOString().split('T')[0]));
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setDateInvested(investor?.dateInvested || new Date().toISOString().split('T')[0]);
  ...
  const payoutDay = useMemo(() => {
    if (!dateInvested) return '';
    const day = new Date(dateInvested).getDate();
  ...
  const metaData = useMemo(() => {
    ...
    const displayAddress = investor.signerAddress || investorAddress;
    if (displayAddress) {
      data.push({ label: "Investor Address", value: displayAddress });
    }

    data.push(
      { label: "Total Investment", value: formatCurrency(Number(principalAmount), currencyCode) },
      { label: "Monthly Return", value: `${returnRate}%` },
      { label: "Date Invested", value: formatDateMD(dateInvested) },
      { label: "Monthly Payout", value: payoutDay },
      { label: "Payout Amount", value: formatCurrency(investor.amountPerMonth, currencyCode) }
    );

    return data;
  }, [fundHolderName, investor.name, investor.signerAddress, investorAddress, dateInvested, investor.amountPerMonth, principalAmount, currencyCode, returnRate, payoutDay]);
  ...
  const handleSharePDF = async () => {
    ...
    try {
      ...
      const base64Data = await generateContractPDFBase64(
        title, 
        termsText, 
        investor.signature, 
        investor.name, 
  ...
  const handlePrint = async () => {
    if (Capacitor.isNativePlatform()) {
      ...
      await generateContractPDF('Investment Agreement', termsText, investor.signature, investor.name, investor.signatureDate ? new Date(investor.signatureDate).toLocaleString() : undefined, metaData, fundHolderName, authorizedSignature);
    }
  };
          ...
          {investor?.signature ? (
            <div className="space-y-6">
              ...
              <div className="space-y-4">
                 ...
                 <button onClick={() => { if (onDeleteSignature) onDeleteSignature(investor.id, 'investor'); }} className="w-full py-3 bg-rose-50 border border-rose-100 text-rose-600 font-bold rounded-2xl active:scale-95 transition-all text-[10px] uppercase tracking-widest flex items-center justify-center gap-2 hover:bg-rose-100">
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-1.5">
                <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1">Investor Address</label>
                <input 
                  type="text" 
                  value={investorAddress} 
                  onChange={e => setInvestorAddress(e.target.value)} 
                  placeholder="Enter current address"
                  className="w-full p-3.5 bg-slate-50 border border-slate-200 rounded-2xl text-sm font-bold outline-none focus:ring-2 focus:ring-blue-500" 
                />
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                 ...
                 <div className="space-y-1.5">
                    ...
                    <input type="date" value={dateInvested} onChange={e => setDateInvested(e.target.value)} className="w-full p-3.5 bg-slate-50 border border-slate-200 rounded-2xl text-sm font-bold outline-none focus:ring-2 focus:ring-blue-500" />
                 </div>
              </div>
              <div className="space-y-1.5">
            ...
            <div className="space-y-6">
              ...
              <div className="flex flex-col gap-2">
                <div className="flex gap-2">
                  <button onClick={handleCopyLink} className={`flex-[2] py-4 font-black rounded-2xl shadow-xl active:scale-95 transition-all text-xs uppercase tracking-widest flex items-center justify-center gap-2 ${copied ? 'bg-emerald-700 text-white' : 'bg-blue-600 text-white'}`}>
