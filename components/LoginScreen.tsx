...
const RECENT_ACTIONS_SHEET_NAME = "RecentActions";
...
function isSystemSheet(sheetName) {
  ...
  return name.startsWith("_") || name.startsWith("report_") || name === HISTORY_SHEET_NAME.toLowerCase() || name === RECENT_ACTIONS_SHEET_NAME.toLowerCase() || name === "earnings" || name === USERS_SHEET_NAME.toLowerCase() || name === INVESTORS_SHEET_NAME.toLowerCase() || name === SIGNATURES_SHEET_NAME.toLowerCase() || name === CONTRACTS_SHEET_NAME.toLowerCase() || name === "main ledger" || name.endsWith(" incoming") || name.endsWith(" outgoing");
}
...
function ensureHeaders(sheet, type) {
    ...
    else if (type === 'investors') { headers = ["ID", "Name", "Bank Name", "Bank Number", "Amount", "Date Invested", "Percent", "Monthly Amount", "Notes"]; }
    ...
    else if (type === 'recent_actions') { headers = ["Timestamp", "Details", "Tab Name"]; color = "#1e293b"; }
...
function getRecords(sheet, type, limit, offset) {
  if (!sheet) return [];
  var data = sheet.getDataRange().getValues();
  ...
  var recordsData = data.slice(SKIP_ROWS);
  if (offset !== undefined && limit !== undefined) {
    recordsData = recordsData.slice(offset, offset + limit);
  } else if (limit !== undefined) {
    recordsData = recordsData.slice(0, limit);
  }

  return recordsData.map(function(row) {
    ...
    else if (type === 'investors') { record.name = row[1] ? row[1].toString() : ""; record.bankName = row[2] ? row[2].toString() : ""; record.bankNumber = row[3] ? row[3].toString() : ""; record.amount = Number(row[4]) || 0; record.dateInvested = formatDateSafe(row[5]); record.percentPerMonth = Number(row[6]) || 0; record.amountPerMonth = Number(row[7]) || 0; record.notes = row[8] ? row[8].toString() : ""; }
...
function cleanupOldRecentActions(ss) {
  var sheet = ss.getSheetByName(RECENT_ACTIONS_SHEET_NAME);
  if (!sheet) return;
  var data = sheet.getDataRange().getValues();
  var now = new Date().getTime();
  ...
  for (var i = data.length - 1; i >= SKIP_ROWS; i--) {
    ...
    if (now - date.getTime() > oneMonthMs) {
      sheet.deleteRow(i + 1);
    }
  }
}

function logRecentAction(ss, details, tabName) {
  if (tabName === 'Investors' || tabName === 'Tab: Investors') return;
  var sheet = ss.getSheetByName(RECENT_ACTIONS_SHEET_NAME);
  if (!sheet) { sheet = ss.insertSheet(RECENT_ACTIONS_SHEET_NAME); ensureHeaders(sheet, 'recent_actions'); }
  safeAppend(sheet, [new Date(), details, tabName || ""]);
}

function doGet(e) {
  var ss = SpreadsheetApp.getActiveSpreadsheet();
  cleanupOldContracts(ss);
  cleanupOldRecentActions(ss);
  ...
  var limit = e.parameter.limit ? parseInt(e.parameter.limit) : undefined;
  var offset = e.parameter.offset ? parseInt(e.parameter.offset) : undefined;
  if (e.parameter.full === 'true') {
    ...
    tabs.forEach(function(t) {
      try { 
        ...
        if (sheet) response.allRecords[t] = getRecords(sheet, type, limit, offset); 
      } catch(err) { response.allRecords[t] = []; }
    });
    var sigSheet = ss.getSheetByName(SIGNATURES_SHEET_NAME);
    if (sigSheet) {
      var sigs = getRecords(sigSheet, 'signatures', limit, offset);
      var sigMap = {};
    ...
    var raSheet = ss.getSheetByName(RECENT_ACTIONS_SHEET_NAME);
    if (raSheet) {
      var raData = raSheet.getDataRange().getValues();
      var actionsByTab = {};
      var initialActions = [];
      for (var i = raData.length - 1; i >= SKIP_ROWS; i--) {
        var details = raData[i][1];
        var rowTabName = raData[i][2] || "";
        if (!details) continue;
        var detailsStr = details.toString();
        
        var tabName = rowTabName;
        if (!tabName) {
            var tabMatch = detailsStr.match(/^\[(.*?)\] /);
            tabName = tabMatch ? tabMatch[1] : "Global";
        }
        
        if (!actionsByTab[tabName]) actionsByTab[tabName] = 0;
        
        if (actionsByTab[tabName] < 20) {
          initialActions.push({ timestamp: raData[i][0], details: detailsStr, tabName: tabName });
          actionsByTab[tabName]++;
        }
      }
      response.initialRecentActions = initialActions;
    }
  } else if (targetTab) {
    try { 
      ...
      if (sheet) response.records = getRecords(sheet, type, limit, offset); 
...
function doPost(e) {
  ...
  if (action === "deleteActionHistory") {
    var sheet = ss.getSheetByName(RECENT_ACTIONS_SHEET_NAME);
    if (sheet) {
      var data = sheet.getDataRange().getValues();
      var targetTimestamp = postData.timestamp;
      var targetDetails = postData.details;
      
      for (var i = data.length - 1; i >= SKIP_ROWS; i--) {
        var rowTimestamp = data[i][0];
        var rowTimestampStr = (rowTimestamp instanceof Date) ? rowTimestamp.toISOString() : String(rowTimestamp);
        var targetTimestampStr = String(targetTimestamp);
        
        if (rowTimestampStr === targetTimestampStr && String(data[i][1]) === String(targetDetails)) {
          sheet.deleteRow(i + 1);
          return ContentService.createTextOutput(JSON.stringify({ status: "success" })).setMimeType(ContentService.MimeType.JSON);
        }
      }
      return ContentService.createTextOutput(JSON.stringify({ status: "error", message: "Action history not found" })).setMimeType(ContentService.MimeType.JSON);
    }
  ...
  if (action === "getRecentActions") {
    var sheet = ss.getSheetByName(RECENT_ACTIONS_SHEET_NAME);
    var actions = [];
    if (sheet) {
        var data = sheet.getDataRange().getValues();
        var tabName = postData.tabName;
        var offset = postData.offset || 0;
        var limit = postData.limit || 20;
        
        var filteredData = [];
        for (var i = data.length - 1; i >= SKIP_ROWS; i--) {
            var details = data[i][1];
            var rowTabName = data[i][2] || "";
            if (!details) continue;
            var detailsStr = details.toString();
            
            var parsedTabName = rowTabName;
            if (!parsedTabName) {
                var match = detailsStr.match(/^\[(.*?)\] /);
                if (match) {
                    parsedTabName = match[1];
                } else {
                    parsedTabName = "Global";
                }
            }
            
            if (!tabName || parsedTabName === tabName || parsedTabName === "Global") {
                filteredData.push({ timestamp: data[i][0], details: detailsStr, tabName: parsedTabName });
            }
        }
        
        actions = filteredData.slice(offset, offset + limit);
    }
    return ContentService.createTextOutput(JSON.stringify({ status: "success", actions: actions })).setMimeType(ContentService.MimeType.JSON);
  }

  if (action === "logRecentAction") {
    logRecentAction(ss, postData.details, postData.tabName);
    return ContentService.createTextOutput(JSON.stringify({ status: "success" })).setMimeType(ContentService.MimeType.JSON);
  }
  ...
  if (action === "deleteRecord") {
    ...
    for (var i = SKIP_ROWS; i < data.length; i++) {
      if (data[i][0] == deletedRecordId) {
        ...
        if (postData.status && (tabType === 'debt' || tabType === 'rent')) {
          var hSheet = ss.getSheetByName(HISTORY_SHEET_NAME);
  ...
  if (action === "addInvestor") {
    ...
    safeAppend(invSheet, [inv.id, inv.name, inv.bankName, inv.bankNumber, inv.amount, inv.dateInvested, inv.percentPerMonth, inv.amountPerMonth, inv.notes || ""]);
  ...
  if (action === "updateInvestor") {
    ...
    for (var i = SKIP_ROWS; i < data.length; i++) {
      if (data[i][0].toString() == inv.id.toString()) {
        sheet.getRange(i + 1, 2, 1, 8).setValues([[inv.name, inv.bankName, inv.bankNumber, inv.amount, inv.dateInvested, inv.percentPerMonth, inv.amountPerMonth, inv.notes || ""]]);
        break;
      }
    }
