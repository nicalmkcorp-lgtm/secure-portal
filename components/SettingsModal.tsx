...
const SettingsModal: React.FC<SettingsModalProps> = ({ onClose, settings, onSave, allTabs, onToggleRestriction, onManageUsers, showToast, isOfflineMode, onPushToServer, onPullFromServer, onGoOnline, forbiddenUrl }) => {
  const [formData, setFormData] = useState<AppSettings>(({
    ...
    copyDateBullet: settings.copyDateBullet || '📌',
  ...
  const handleSave = async () => {
    ...
    // Add a small delay to make the saving feedback visible
    setTimeout(() => {
  ...
  return (
    ...
    >
              ...
              <section className="space-y-4">
                ...
                <div className="p-6 bg-slate-50 border border-slate-100 rounded-[2.5rem] space-y-4 shadow-inner">
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div className="space-y-2">
                      <label className="text-[9px] font-bold text-slate-400 uppercase tracking-widest ml-1">Date Bullet</label>
                      <div className="relative">
                        <span className="absolute left-4 top-1/2 -translate-y-1/2 opacity-30 pointer-events-none"><PaletteIcon /></span>
                        <input type="text" className="w-full px-4 py-4 bg-white border border-slate-200 rounded-2xl text-3xl font-black text-center shadow-inner outline-none focus:border-blue-500 focus:ring-4 focus:ring-blue-500/5 h-[56px]" value={formData.copyDateBullet} onChange={e => setFormData({ ...formData, copyDateBullet: e.target.value })} placeholder="📌" />
                      </div>
                    </div>
                  </div>
                  <div className="space-y-2">
      ...
      {isSavingSettings && <LoadingOverlay isVisible={true} message="Saving Settings..." color={formData.loadingColor} />}
...
const SignaturePadModal = ({ isOpen, onClose, onSave }: { isOpen: boolean, onClose: () => void, onSave: (data: string) => void }) => {
  ...
  const trimCanvas = (sourceCanvas: HTMLCanvasElement) => {
    ...
    if (trimmedContext) {
      trimmedContext.drawImage(sourceCanvas, left, top, trimWidth, trimHeight, 0, 0, trimWidth, trimHeight);
    }
  ...
  const handleSave = () => {
    const canvas = canvasRef.current;
    if (canvas) {
      // Trim the canvas to remove excessive white space around the signature
      const trimmedCanvas = trimCanvas(canvas);
      
      const tempCanvas = document.createElement('canvas');
      const maxDim = 400; // Limit dimension for cloud storage
      const scale = Math.min(maxDim / trimmedCanvas.width, maxDim / trimmedCanvas.height, 1);
