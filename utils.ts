...
export const syncInvestorReminders = async (investors: Investor[]) => {
  try {
    ...
    const notificationsToSchedule = investors.map((inv, idx) => {
      const investDate = new Date(inv.dateInvested);
      if (isNaN(investDate.getTime())) return null;

      // Calculate the next occurrence
      const now = new Date();
      let nextDate = new Date(investDate);
      ...
      const id = parseInt(`99${idx}`);

      return {
        title: 'Investor Payout Reminder',
...
const buildContractDoc = (title: string, content: string, signatureBase64: string | undefined, signerName: string, dateSigned: string | undefined, metaData: any[] = [], authorizedSigner: string = "", authorizedSignatureImage?: string) => {
  ...
  if (authorizedSignatureImage) {
    try { 
      doc.addImage(authorizedSignatureImage, 'PNG', centerLeft - 25, sigBlockY - 12, 50, 25); 
    } catch (e) {}
  }
  doc.setFont("times", "normal");
  ...
  // Right (Client/Investor) - Perfectly aligned to right margin
  ...
  if (signatureBase64) {
      try { 
        doc.addImage(signatureBase64, 'PNG', centerRight - 32, sigBlockY - 18, 64, 32); 
      } catch (e) {}
  }
  doc.setFont("times", "normal");
